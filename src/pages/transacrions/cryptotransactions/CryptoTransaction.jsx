...
const CryptoTransaction = () => {
  ...
  const getData = async (page = 1, pageSize = resultsPerPage) => {
    ...
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/deposite/admin/transactions/`, {
        ...
        <div className="col-lg-9 pt-4 pb-2 pb-sm-4">
          ...
          <form onSubmit={handleFilterSubmit} className="mb-4">
            <div className="row g-3">
              ...
              <div className="col-md-3">
                <TextField label="Start Date" type="date" InputLabelProps={{ shrink: true }} value={startDate} onChange={(e) => setStartDate(e.target.value)} fullWidth size="small" />
              </div>
              <div className="col-md-3">
                <TextField label="End Date" type="date" InputLabelProps={{ shrink: true }} value={endDate} onChange={(e) => setEndDate(e.target.value)} fullWidth size="small" />
              </div>
              <div className="col-md-2 d-flex align-items-end">
                <Button variant="contained" color="primary" type="submit" fullWidth>Apply</Button>
              </div>
              <div className="col-md-2 d-flex align-items-end">
                <Button variant="outlined" onClick={resetFilters} fullWidth>Reset</Button>
              </div>
            </div>
          </form>
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      transactions.map((txn, idx) => (
                        <tr key={txn.id}>
                          <td>{(pagination.current_page - 1) * resultsPerPage + idx + 1}</td>
                ...
                <div className="container-fluid">
                  <div className="row">
                    ...
                    <div className="col-9 d-flex justify-content-end">
                      <Pagination count={pagination.total_pages} page={pagination.current_page} onChange={handlePageChange} color="primary" />
                    </div>
                  </div>
                </div>
                        ...
                        <TableBody>
                          {Object.entries(selectedTransaction).map(([key, value]) => (
                            <TableRow key={key}>
                              ...
                              <TableCell>{typeof value === "object" ? JSON.stringify(value, null, 2) : value}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
