      ...
      <div className="row pt-sm-2 pt-lg-0">
        ...
        <div className="col-lg-9 pt-4 pb-2 pb-sm-4">
          <div className="d-sm-flex align-items-center mb-4">
            <h1 className="h2 mb-4 mb-sm-0 me-4">Mobile Recharge Transaction</h1>
          </div>
          ...
          <form onSubmit={handleFilterSubmit}>
            <div className="row g-3 mb-4">
              ...
              <div className="col-md-2 d-flex align-items-end">
                <Button variant="outlined" color="light" onClick={resetFilters} fullWidth>
                  Reset
                </Button>
              </div>
            </div>
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      transactions.map((v, i) => (
                        <tr key={i}>
                          <td>{(pagination.current_page - 1) * resultsPerPage + i + 1}</td>
                          <td>{dayjs(v.created_at).format("DD/MM/YYYY hh:mm A")}</td>
                          <td>{v.operator_code}</td>
                          <td>{v.user_details?.full_name}</td>
                          <td>{v.user_details?.email}</td>
                          <td>{v.user_details?.phone_number}</td>
                          <td>
                            <span className={`badge bg-${v.status === "success" ? "success" : "danger"}`}>
                              {v.status}
                            </span>
                          </td>
                          <td>$ {v.amount}</td>
                          <td>
                            <Tooltip title="View">
                              <IconButton color="info" onClick={() => handleOpenModal(v)}>
                ...
                <Table>
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell>{selectedTransaction.user_details.full_name}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Email</strong></TableCell>
                      <TableCell>{selectedTransaction.user_details.email}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Mobile</strong></TableCell>
                      <TableCell>{selectedTransaction.user_details.phone_number}</TableCell>
                    </TableRow>
                    <TableRow>
                    ...
                    <TableRow>
                      ...
                      <TableCell>{selectedTransaction.order_id}</TableCell>
                    </TableRow>
                    <TableRow>
                    ...
                    <TableRow>
                      <TableCell><strong>Recharge Type</strong></TableCell>
                      <TableCell>{selectedTransaction.recharge_type}</TableCell>
                    </TableRow>
                    <TableRow>
                    ...
                    <TableRow>
                      ...
                      <TableCell>{dayjs(selectedTransaction.created_at).format("DD/MM/YYYY hh:mm A")}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              )}
